...
sdResult sdYZPlane(vec3 p, float z, int matID){
    sdResult res;
    res.distance = abs(p.x-z);
    res.materialID = matID;
    return res;
}
...
sdResult sceneSDF(vec3 samplePoint) {
    ...
    res = sdMin( 
        sdMin(
            sdMin( 
                sdXYPlane(samplePoint,-2,0),
                sdSphere(samplePoint-camPos, CAMRADIUS,3)
            ),
            sdMin(
                 sdYZPlane(samplePoint,-2,0),
                 sdXYPlane(samplePoint,2,0)
            )
        ),
        sdMin(
          sdBox(samplePoint,vec3(1,1,2),3),
...
void main() {
    ...
    mats[0].reflectivity = 0;

    mats[1].color = vec3(1,0,0);
    mats[1].reflectivity = 0;

    mats[2].color = vec3(0,.7,0);
    mats[2].reflectivity = 0;
    
    //"Camera"

    mats[3].color = vec3(1,1,1);
    mats[3].reflectivity = reflectMix;
